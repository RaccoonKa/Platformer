import pygame
from game.menu_classes.menu_animations import GlitchEffect
from game.menu_classes.menu_objects import Objects
from game.menu_classes.menu_music import Music
from game.menu_things.secrets import setup_secrets
from game.menu_things.menu_ui import (
...
def menu(sound_volume_ : float, music_volume_ : float):
    #pygame.init()

    # Setting the screen resolution
    GAME_WIDTH, GAME_HEIGHT = 1920, 1080
    #info = pygame.display.Info()
    SCREEN_WIDTH, SCREEN_HEIGHT = GAME_WIDTH, GAME_HEIGHT #info.current_w, info.current_h
    ...
    # Launch music
    music = Music(volume= music_volume_)
    music.play_loop()

    # Create menu elements
    title, press_any_button, menu_items = create_main_menu_elements(GAME_WIDTH, GAME_HEIGHT)
    settings_title, music_slider, sound_slider, back_button = create_settings_elements(GAME_WIDTH, GAME_HEIGHT, music, sound_volume_)
                        ...
                        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                            if next_button.bg_rect.collidepoint(game_x, game_y):
                                ...
                                if current_image_index > len(game_images):
                                    return (SCREEN_WIDTH, SCREEN_WIDTH), 60, screen, clock, 1, sound_volume, music.volume, False
                                    # print("Starting actual game...") #todo
                                    # current_state = "main_menu"
                                    # background_manager.start()
                                    # music.play_loop()
                                    # game_start_active = False
    ...
    #todo
    return (SCREEN_WIDTH, SCREEN_WIDTH), 60, screen, clock, 1, sound_volume, music.volume, True
